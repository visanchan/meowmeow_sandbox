...
function ChargeModal({ open, total, onClose, onConfirm }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      background: "rgba(43,35,29,.42)", backdropFilter: "blur(6px)",
      display: "flex", alignItems: "center", justifyContent: "center",
      animation: "fade 180ms ease",
    }}>
      <div onClick={e => e.stopPropagation()} style={{
        width: 480, background: "var(--panel-strong)", borderRadius: 32,
        boxShadow: "var(--shadow-pop)", padding: 28,
      }}>
        <span className="eyebrow">Charge</span>
          ...
          {TENDERS.map(t => {
            ...
            return (
              <button key={t.id} onClick={() => setTender(t.id)} style={{
                ...
                background: sel ? "var(--grad-button)" : "var(--soft)",
        ...
        {tender === "cash" && (
          <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 6 }}>
            <label style={{ fontSize: 11, fontWeight: 800, letterSpacing: ".06em", textTransform: "uppercase", color: "var(--muted-warm)" }}>Cash received</label>
            <input
              type="number"
              value={received}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", color: "var(--muted)", fontSize: 13, marginTop: 4 }}>
              <span>Change</span>
              <Money value={change} size={16} weight={800} />
            </div>
          </div>
        )}
...
function ReceiptToast({ open, txid, onDone }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ width: 36, height: 36, borderRadius: 12, background: "var(--success-soft)", color: "var(--success)", display: "grid", placeItems: "center" }}>
      ...
      <div>
        <div style={{ fontWeight: 800, fontSize: 14 }}>Sale recorded</div>
