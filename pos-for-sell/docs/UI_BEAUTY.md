...
4. ✅ **Illustrated empty states** — `EmptyState` gained an optional `icon` (soft lavender circle); every list empty now carries a tasteful **lucide-react** icon: products (Tags), customers (PawPrint), send-later (Package), pre-orders (Clock), correction (ReceiptText), audit-log (ClipboardList). *(pass 11; emoji → lucide-react in the Codex cleanup pass)*
