...
- Internal Dashboard uses a manager-focused cream/brown layout: event total vs goal, goal progress bar, remaining pace, a Day View picker (Day 1→Day 4 tabs with live/closed status), 4-day horizontal pace timeline (numbered Day 1→Day 4 dots with connector and per-day total/receipts/items cells underneath), Today By Hour bars (`<10`, `10`-`20`, `>21`), today stats, today payment split, Top Sellers card (paid items only, top 5 horizontal bars by units sold then revenue), and Low Stock Alerts card (SKUs whose remaining is at or below their low-stock threshold on the active day, sorted by remaining ascending).
- The Day View picker drives every "today"-labeled widget on the dashboard: Today header, Today By Hour, Payment split, Receipts/Items/Avg Bill tiles, and Low Stock Alerts. The picker defaults to the live operating day (live behavior unchanged on first open) and lets the manager click any past or future day to inspect that day's snapshot. Cumulative widgets (Event total, Goal, 4-Day Pace timeline, Top Sellers · Event) stay event-wide regardless of selection.
- The Day View hint shows whether the manager is viewing the live operating day, a closed day, or a future day. The Today, Today By Hour, and Payment split titles swap to `Viewing · Day X`, `Day X By Hour`, and `Payment split · Day X` when a non-current day is selected so the historical context is unambiguous.
...
- Top Sellers excludes free gifts and Send Later preorders; revenue per row is summed `lineTotal` across the event.
