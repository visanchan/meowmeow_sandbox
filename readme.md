...
- Bill Correction also exposes a `Void Bill` action for the rare case where a bill should be deleted entirely (the edit flow's "at least one bill item must remain" guard still applies, so it cannot be used for deletion).
- Voiding a bill requires a reason in a confirm dialog, removes the sale from saved sales, and re-aligns inventory carry-forward across later days the same way a correction does.
...
- Voiding cannot be undone except by re-creating the sale.
