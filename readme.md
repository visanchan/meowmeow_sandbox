...
- Internal Dashboard uses a manager-focused cream/brown layout: event total vs goal, goal progress bar, remaining pace, 4-day horizontal pace timeline (numbered Day 1→Day 4 dots with connector and per-day total/receipts/items cells underneath), today stats, today payment split, Top Sellers card (paid items only, top 5 horizontal bars by units sold then revenue), and Low Stock Alerts card (SKUs whose remaining is at or below their low-stock threshold on the active day, sorted by remaining ascending).
